...
class Jax2TfLimitation(primitive_harness.Limitation):
  ...
  def svd(cls, harness: primitive_harness.Harness):
    ...
    def custom_assert(tst, r_jax, r_tf, *, args, tol, err_msg):
      ...
      # should also consider the gap between it and zero. Note that this
      ...
      def compare_singular_vectors(x, y, *, error_bound):
        ...
        cos_angular_diff = (dot_column_wise(x, y) /
                            (jnp.sqrt(dot_column_wise(x, x)) *
                             jnp.sqrt(dot_column_wise(y, y))))
        ...
        # values like 1.0000001 on float32, which are clipped to 1.0.
        cos_angular_diff = jnp.clip(cos_angular_diff, a_min=0.0, a_max=1.0)

        angular_diff = jnp.arccos(cos_angular_diff)

        # TODO: figures out the numerical error accumulations through the ops
        # `sqrt`, `divide`, and `arccos` and removes the scaling factor on the
        # angular difference.
        if x.dtype in [np.float32, np.complex64]:
          tol_angular_diff = 1E-4
        elif x.dtype in [np.float64, np.complex128]:
          tol_angular_diff = 1E-9
        np.testing.assert_array_less(tol_angular_diff * angular_diff,
                                     error_bound)
